...
export function mediaDisplayUrl(ref: string) {
  return window.armin?.media?.url(ref) ?? ref;
}

export function mediaRefFromDisplayUrl(url: string) {
  return window.armin?.media?.refFromUrl(url) ?? null;
}

export function resolveMediaRefsInMarkdown(markdown: string) {
  return markdown.replace(MEDIA_REF_RE, (ref) => mediaDisplayUrl(ref));
}

export function persistMediaRefsInMarkdown(markdown: string) {
