    ...
    <div>
      {setupQuery.isError && (
        <div className="flex items-center justify-between gap-4 border-b border-border px-4 py-3.5">
          <p className="text-sm text-muted">Couldn&apos;t load MCP setup info.</p>
          <Button variant="outline" size="sm" onClick={() => void setupQuery.refetch()}>
            Try again
          </Button>
        </div>
      )}
        ...
        <>
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-6 border-b border-border px-4 py-3.5">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[0.8125rem] text-muted">
                Point <code className="font-mono text-ink">ARMIN_DATA_DIR</code> here so
                agents write to the same database as this profile.
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-6 border-b border-border px-4 py-3.5">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-mono text-[0.8125rem] text-ink">{setup.profileId}</p>
              <p className="mt-0.5 text-[0.8125rem] text-muted">
                Set <code className="font-mono text-ink">ARMIN_PROFILE_ID</code> to this value.
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          {setup.isPackaged && (
            <div className="border-b border-border px-4 py-3.5">
              <p className="text-[0.8125rem] leading-snug text-muted">
                ...
                <code className="font-mono text-ink">/absolute/path/to/armin</code> in the
                snippets below with that path.
              </p>
            </div>
          )}
          ...
          <div className="border-b border-border px-4 py-3.5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Segmented options={AGENT_TABS} value={agent} onChange={setAgent} size="sm" />
            </div>
            {agentSnippet && (
              <>
                <p className="mt-1 text-[0.8125rem] text-muted">{agentSnippet.hint}</p>
                <div className="mt-3 flex items-start gap-2 border border-border bg-surface-sunken py-2 pl-3 pr-2">
                  ...
                  <Button
                    ...
                    onClick={() => void copy(agentSnippet.body, agentSnippet.title)}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
          ...
          <div className="px-4 py-3.5">
            <p className="text-[0.8125rem] leading-snug text-muted">
              The MCP server uses stdio and starts when your agent connects. Cards created
              through MCP appear here after a refresh.
            </p>
          </div>
        </>
      )}
